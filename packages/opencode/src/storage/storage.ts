...
export namespace Storage {
  ...
  class LanceDBManager {
    ...
    private async performInit(): Promise<void> {
      try {
        ...
        log.error("Failed to initialize LanceDB", { error, stack: (error as Error).stack, message: (error as Error).message });
  ...
  export async function searchByVector(queryVector: number[], limit: number = 10): Promise<Array<{key: string[], value: any, score: number}>> {
    const table = await LanceDBManager.getInstance().getTable();
  ...
  export async function search<T = any>(prefix: string[], query?: string, options?: { limit?: number }): Promise<Array<{ key: string[], value: T, score?: number }>> {
