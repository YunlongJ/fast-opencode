...
export class CompactionService {
   ...
   */
  public static async isOverflow(input: { tokens: MessageV2.Assistant["tokens"]; model: Provider.Model }): Promise<boolean> {
    ...
    const outputLimit = Math.min(input.model.limit.output, SessionPrompt.OUTPUT_TOKEN_MAX) || SessionPrompt.OUTPUT_TOKEN_MAX;
    const usable = input.model.limit.input || context - outputLimit;
    ...
    try {
      ...
      const result = await LLM.generate({
        ...
        messages: [
          {
            role: "system",
            content: "You are a tool output summarizer. Summarize the following tool output concisely, preserving key findings, errors, and file paths. Keep it under 200 words.",
          },
