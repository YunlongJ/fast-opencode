...
export class StepEngine {
  private static readonly log = Log.create({ service: "step.engine" })
  ...
  async run(streamInput: LLM.StreamInput): Promise<StepEngineResult> {
    ...
    try {
      for (let toolStep = 0; toolStep < maxToolSteps; toolStep++) {
        ...
        for await (const value of stream.fullStream) {
          ...
          switch (value.type) {
            ...
            case "finish-step": {
              ...
              // Check token overflow but don't interrupt current step
              // Let current step complete, handle compaction in next round
              if (await CompactionService.isOverflow({ tokens: usage.tokens, model: this.input.model })) {
                needsCompaction = true
                // Don't break here - let current step finish
              }
              break
            }
            case "text-start":
            ...
            case "text-end":
              if (currentText) {
                ...
                // Remove memory markers from display text
                currentText.text = currentText.text.replace(/\[MEMORY:\w+:[^\]]+\]/g, "")
        ...
        // Check if we need compaction before executing tools
        // If so, save pending plan and return
        if (needsCompaction && plan.nodes.length > 0) {
          const pendingNodes = plan.nodes.filter((n) => n.status === "pending")
          if (pendingNodes.length > 0) {
            // Save pending plan info to blackboard for later recovery
            this.deps.blackboard.put({
              toolCallId: "pending-plan",
              toolName: "plan-saved",
              input: {},
              ok: true,
              output: JSON.stringify({
                pendingTools: pendingNodes.map((n) => ({
                  id: n.id,
                  toolName: n.toolName,
                  args: n.args,
            ...
            StepEngine.log.warn("Token overflow detected, saved pending plan", {
              sessionID: this.input.sessionID,
              pendingTools: pendingNodes.length,
            })
          }
          return { blocked, needsCompaction: true, metrics }
        }
        metrics.plannedNodes += plan.nodes.length
  ...
  private injectContext(messages: ModelMessage[], contextStr: string): ModelMessage[] {
    const systemMsg = messages.find((m) => m.role === "system")
    if (systemMsg && typeof systemMsg.content === "string") {
      const newContent = systemMsg.content + "\n\n【记忆上下文】\n" + contextStr
      return messages.map((m) => (m === systemMsg ? { ...m, content: newContent } : m))
    }
    return [{ role: "system", content: "【记忆上下文】\n" + contextStr }, ...messages]
  }

  private parseMemoryMarkers(text: string): Array<{
    type: "decision" | "todo" | "change"
    content: string
    keywords?: string[]
  }> {
    ...
    // Match [MEMORY:type:content:keywords]
    const regex = /\[MEMORY:(\w+):([^:]+)(?::([^\]]+))?\]/g
    let match

    while ((match = regex.exec(text)) !== null) {
      const [, type, content, keywordsStr] = match

      if (type === "decision" || type === "todo" || type === "change") {
        memories.push({
          type,
